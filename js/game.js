...
const INVINCIBILITY_DURATION = 3000; // 3 segundos en milisegundos <-- Declarada aquí una sola vez
...
let ai = null; // Instancia de la IA
...
function updateScoreDisplay() {
    ...
    if (scoreDisplay) {
        scoreDisplay.innerHTML = `Vidas: ${players.filter(p => !p.isMalo).map(p => `Niño ${p.id}: ${p.lives}`).join(' | ')}`;
    }
}
...
function startGame() {
    ...
    if (controlsDisplay) {
        controlsDisplay.style.display = 'none'; // Ocultar controles al empezar
    }
    const scoreDisplay = document.getElementById('score-display');
     if (scoreDisplay) {
        scoreDisplay.textContent = "¡El juego ha comenzado! ¡Corre!";
     }
...
function handlePlayerMovement(deltaTime) {
    players.forEach(player => {
        if (player.isEliminated || player.isMalo) return; // No mover si está eliminado o es el
